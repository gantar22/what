...
public class Blue_Script : MonoBehaviour {
	...
	void Update () {
		Vector3 velocity = new Vector3 (Input.GetAxis("Horizontal_B"),0f,0f);
		transform.Translate(velocity * Time.deltaTime);
		if(Physics2D.Raycast(transform.position + new Vector3(0,-.11f,0),Vector3.down,.07f,~(1<<8)).collider != null && Input.GetButton("Jump_B")
			&& jumpDeadTime == 0)
		{
